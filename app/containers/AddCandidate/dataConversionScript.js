...
export const shapeDataForSnapshot = (candidateData = {}) => {
  let dataTemplate = {
    name: "",
    profileForPosition: "",
    designation: "",
  ...
  dataTemplate["name"] = candidateData.name != "" ? candidateData.name : "-";
  dataTemplate["profileForPosition"] = "Test Development";
  dataTemplate["designation"] =
    candidateData.experiences[0].designation != ""
      ? candidateData.experiences[0].designation
      : "-";
  dataTemplate["reportingRelations"] = `Reporting to ${
    candidateData.reportingTo
  } \n Reportees - ${candidateData.noOfReportees}`;
  dataTemplate["qualification"] = candidateData.qualification;
  dataTemplate["presentLocation"] = candidateData.presentLocation || "-";
  dataTemplate["location"] =
    candidateData.presentLocation != ""
      ? `${candidateData.presentLocation} / ${candidateData.nativeLocation}`
      : "-";
  dataTemplate["salary"] = {
    currentCTC:
      candidateData.presentCTC != ""
        ? `${candidateData.presentCTC} ${candidateData.presentCTCRemarks}`
        : "-",
    expectedCTC:
      candidateData.expectedCTC != ""
        ? `${candidateData.expectedCTC} ${candidateData.expectedCTCRemarks}`
        : "-",
    noticePeriod:
      candidateData.noticePeriod != ""
        ? `${candidateData.noticePeriod} days ${
            candidateData.noticePeriodRemarks
          }`
        : "-",
  };
  dataTemplate["yearsOfExperience"] =
    candidateData.yearsOfExperience != ""
      ? `${candidateData.yearsOfExperience} years ${
          candidateData.monthsOfExperience
        } months`
      : "-";
  dataTemplate["careerHighlights"] = candidateData.experiences;
  dataTemplate["personalDetails"] = {
    dob:
      candidateData.dateOfBirth != ""
        ? parseDate(candidateData.dateOfBirth, "DD MMM YYYY")
        : "-",
    gender: candidateData.gender != "" ? candidateData.gender : "-",
    maritalStatus:
      candidateData.maritalStatus != "" ? candidateData.maritalStatus : "-",
  };
  dataTemplate["consultantAssessment"] =
    candidateData.consultantAssessment != "" ||
    candidateData.reasonForChange != ""
      ? `${candidateData.consultantAssessment} \n \n Reason for change: ${
          candidateData.reasonForChange
        }`
      : "-";
